...
resource "aws_iam_role_policy" "LambdaInvokePolicy" {
  name = "${local.name}-LambdaInvokePolicy"
...
resource "aws_iam_role" "import_licenses_task_role" {
  assume_role_policy = <<POLICY
{
  "Statement": [
    {
...
POLICY
  ...
  max_session_duration = "3600"
  name                 = "${local.name}-import-licenses-TaskRole"
  path                 = "/"

  tags = local.common_tags
}

resource "aws_iam_role_policy" "import_licenses_policy1" {
  name = "${local.name}-import-licenses-from-infrastructureSecretsPolicy1"
...
POLICY
  ...
  role = aws_iam_role.import_licenses_task_role.name
}

resource "aws_iam_role" "import_licenses_task_execution_role" {
  assume_role_policy = <<POLICY
{
  "Statement": [
    {
...
POLICY
  ...
  inline_policy {
    name   = "import-licenses-from-infrastructureSecretsPolicy"
    policy = <<POLICY
{
  "Version":"2012-10-17",
  "Statement":[
...
POLICY
  ...
  max_session_duration = "3600"
  name                 = "${local.name}-import-licenses-from-in-ExecutionRole"
  path                 = "/"

  tags = local.common_tags
}

resource "aws_iam_role_policy" "import_licenses_policy2" {
  name = "${local.name}-import-licenses-from-infrastructureSecretsPolicy2"
...
POLICY
  ...
  role = aws_iam_role.import_licenses_task_execution_role.name
}


resource "aws_iam_role" "export_rds_to_s3_task_role" {
  assume_role_policy = <<POLICY
{
  "Statement": [
    {
...
POLICY
  ...
  max_session_duration = "3600"
  name                 = "${local.name}-export-rds-to-s3-TaskRole"
  path                 = "/"

  tags = local.common_tags
}

resource "aws_iam_role_policy" "export_rds_to_s3_policy1" {
  name = "${local.name}-export-rds-to-s3-SecretsPolicy1"
...
POLICY
  ...
  role = aws_iam_role.export_rds_to_s3_task_role.name
}

resource "aws_iam_role" "export_rds_to_s3_task_execution_role" {
  assume_role_policy = <<POLICY
{
  "Statement": [
    {
...
POLICY
  ...
  inline_policy {
    name   = "export-rds-to-s3-SecretsPolicy"
    policy = <<POLICY
{
  "Version":"2012-10-17",
  "Statement":[
...
POLICY
  ...
  managed_policy_arns  = ["arn:aws:iam::aws:policy/service-role/AmazonECSTaskExecutionRolePolicy"]
  max_session_duration = "3600"
  name                 = "${local.name}-export-rds-to-s3-ExecutionRole"
  path                 = "/"

  tags = local.common_tags
}

resource "aws_iam_role_policy" "export_rds_to_s3_policy2" {
  name = "${local.name}-export-rds-to-s3-SecretsPolicy2"
...
POLICY
  ...
  role = aws_iam_role.export_rds_to_s3_task_execution_role.name
}
